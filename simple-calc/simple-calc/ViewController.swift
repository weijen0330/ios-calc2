...
class ViewController: UIViewController{
    ...
    @IBAction func operation(sender: UIButton) {
        ...
        if (lastOp == "") {
            /* If user just pressed an operation */
            if (currNumString == "") {
        ...
        /* pressed operation again */
            ...
            /* simply changes to a new operation */
                ...
                lastOp = sender.titleLabel!.text!
                display.text = lastOp
                let index = historyString.endIndex.advancedBy(-2)
                historyString = historyString.substringToIndex(index) + display.text! + " "
            }
        } else {
            numCounter += 1
            switch lastOp {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        destViewController.historyString = historyString
    }
}
